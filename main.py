...
def getItems(account_id, item_id):
    ...
    if item_id is None:
        result = connection.execute(
            ...
            .where(table.c['ACCOUNT_ID'] == request.view_args['account_id'])).fetchall().mappings().to_json(orient='records')
        logging.info(result)
    else:
        result = connection.execute(
            table.select().join(delete_table, table.c['ACCOUNT_ID'] == delete_table.c['ACCOUNT_ID'] and table.c['ID'] == delete_table.c['ID'] ,isouter=True, full=False)
            .where(table.c['ACCOUNT_ID'] == request.view_args['account_id'] and table.c['ID'] == item_id)).fetchall().mappings().to_json(orient='records')
        logging.info(result)
